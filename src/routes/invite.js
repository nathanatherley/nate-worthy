...
// template (toEmail, fromName, shareUrl, pickRestaurant are the only
// client-supplied values, never free-text body content), so this can't
// become an open email relay for spamming arbitrary content to third
// parties.
...
router.post('/', async (req, res) => {
  ...
  const pickRestaurant = (req.body.pickRestaurant || '').trim().slice(0, 200) || null;
  ...
  try {
    await sendInviteEmail({ toEmail, fromName, shareUrl, pickRestaurant });
    res.json({ sent: true });
  } catch (e) {
